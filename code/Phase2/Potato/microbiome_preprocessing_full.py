"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   POTATO MICROBIOME — UNIFIED PREPROCESSING + GRAPH CONSTRUCTION PIPELINE  ║
║   nzCLR → 8 DR Methods + 2 Graph Types                                     ║
║                                                                              ║
║   Dataset   : PotatoSCMP (PotatoSCMP.csv)                                   ║
║   Features  : BF_g_* columns (885 bacterial genus features)                 ║
║   Label     : Soil_type                                                      ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
# ─────────────────────────────────────────────────────────────────────────────
CONFIG = {
    "abundance_csv"        : "PotatoSCMP.csv",
    "output_dir"           : "preprocessed",
    # Potato dataset specifics
    "feature_col_prefix"   : "BF_g_",     # bacterial genus features
    "label_col"            : "Soil_type",
...
def load_and_validate(cfg):
    ...
    # Drop all-zero columns (uninformative features)
    nonzero_mask = (abundance != 0).any(axis=0)
    abundance    = abundance.loc[:, nonzero_mask]
    _log("features after zero-col drop", str(abundance.shape[1]))
    ...
    # Sample IDs
    sample_id_col = next((c for c in df.columns if c.lower() in ["sample", "sample_id", "sampleid"]), None)
    if sample_id_col:
...
def run_preprocessing_pipeline(cfg=CONFIG):
    print("\n" + "█"*64)
    print("  POTATO MICROBIOME — UNIFIED PREPROCESSING PIPELINE")
  ...
  Outputs → {out_dir}/
    ...
    y_true.npy             Soil type label integers
