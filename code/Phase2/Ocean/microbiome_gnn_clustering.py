...
def main():
    ...
    # Save label arrays so evaluation script can visualise GNN clusters on DR embeddings
    for name, _ in experiments:
        if name == "KNN+DMoN":
            labels_dict[name] = labels_knn_dmon.tolist()
        elif name == "KNN+MinCutPool":
            labels_dict[name] = labels_knn_mincut.tolist()

    labels_path = f"{out_dir}/gnn_labels.json"
    with open(labels_path, "w") as f:
        json.dump(labels_dict, f, indent=4)
    print(f"Saved label arrays to {labels_path}")
